...
###############################################################
# Image Pre-Processing Operations to improve output accurracy #
###############################################################
...
def calculate_ss_confidence(ss_details):
...
def is_valid_path(path):
    ...
    if os.path.isfile(path):
        ...
        # return path
        raise ValueError("Invalid as Entered a Folder Path {path}")
    else:
...
def ocr_img(
    ...
    # If image file is inputted as a parameter
    if input_file:
...
def ocr_file(**kwargs):
    ...
    for pg in range(pdfIn.pageCount):
        ...
        upd_np_array, pg_readable_items, pg_matches, pg_total_confidence, pg_output_data \
            = ocr_img(img=img, input_file=None, search_str=search_str  # False
    ...
    summary = {
        "File": input_file, 
        "Total pages": int(pdfIn.pageCount), 
        "Processed pages": int(dfResult['page'].count()), 
        "Total readable words": int(dfResult['page_readable_items'].sum()),
        "Search string": search_str,
        "Total matches": int(dfResult['page_matches'].sum()), 
        "Confidence score": float(dfResult['page_total_confidence'].mean()), 
    ...
    if output_file:
        ...
        pdfOut.save(output_file_path)
    pdfOut.close()

    page_stats = dfResult.to_dict('records')
    return summary, page_stats


# ocr_file(input_file='/Users/Amila/Downloads/Projects/Lucid Engen/test_data/new.pdf', 
